...
# FIXME: If it's a 64-bit machine, build the 64-bit image?
# Check the arch of the machine we're running on. If it's 64-bit, use a 32-bit base image instead
case "$(uname -m)" in
  x86_64|aarch64)
    BASE_IMAGE=i386/debian:${DEB_VER}
    ;;
  *)
    BASE_IMAGE=debian:${DEB_VER}
    ;;
esac
...
time ${DOCKER} run \
  ...
  bash -e -o pipefail -c "
    ...
    # binfmt_misc is sometimes not mounted with debian ${DEB_VER} image
